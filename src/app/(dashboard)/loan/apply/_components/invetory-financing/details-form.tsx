...
import { InventoryFinancingSchema, InventoryFinancingSchemaType } from '../../_validation';
...
function InventoryFinancingApplicationForm({ onNext, setData, defaultData }: Props) {
  ...
  const {
    ...
    clearErrors,
    watch,
  } = useForm<InventoryFinancingSchemaType>({
    mode: 'all',
  ...
  const onSubmit: SubmitHandler<InventoryFinancingSchemaType> = data => {
          ...
          <Controller
            ...
            render={({ field }) => (
              <MuiSelectComponent
                ...
                errorMessage={errors.transactionType?.message as string}
                value={field.value ? field.value : ''}
                variant="filled"
...
export default InventoryFinancingApplicationForm;
